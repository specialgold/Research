...
# import environment
# import parameters
import parameters_RCPSP as parameters
...
def get_traj(test_type, pa, env, episode_max_length, pg_resume=None, render=False):
    ...
    for _ in xrange(episode_max_length):
        ...
        if test_type == 'PG':
            ...
            # print('Tetris')
            a = other_agents.get_packer_action(env.renew, env.job_slot)
            # print("tetris a: "+ str(a))

        elif test_type == 'SJF':
            # print('SJF')
            a = other_agents.get_sjf_action(env.renew, env.job_slot)
...
def launch(pa, pg_resume=None, render=False, plot=False, repre='image', end='no_new_job'):
    ...
    # print("test: queue jobslot: " + str(len(env.job_slot.slot)))
    ...
    for seq_idx in xrange(pa.num_ex):
        ...
        for test_type in test_types:
            ...
            job_len = np.array([info.record[i].durations[0] for i in xrange(len(info.record))])
            job_total_size = np.array([np.sum(info.record[i].re_requests[0]) for i in xrange(len(info.record))])
