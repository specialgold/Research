...
def get_packer_action(machine, job_slot):
        ...
        for i in xrange(len(job_slot.slot)):
            ...
            if new_job is not None:  # there is a pending job
                ...
                avbl_res = machine.avbl_slot[:new_job.durations[0], :]
                # print(avbl_res)
                res_left = avbl_res - new_job.re_requests[0]

                if np.all(res_left[:] >= 0):  # enough resource to allocate

                    tmp_align_score = avbl_res[0, :].dot(new_job.re_requests[0])
        ...
        for i in xrange(len(job_slot.slot)):
            ...
            if new_job is not None:  # there is a pending job
                ...
                if np.all(res_left[:] >= 0):  # enough resource to allocate
                    ...
                    tmp_sjf_score = 1 / float(new_job.durations[0])
