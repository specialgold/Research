...
def get_traj(agent, env, episode_max_length, render=False):
    ...
    for _ in xrange(episode_max_length):
        ...
        if done:
            if total % 10000 == 0:
                print("steps: {}, opt30: {}".format(_, env.pa.opt[env.pa.file]))

                print('reward: ' + str(rew)+', path: ' + env.pa.path)
                for index in range(len(info.record)):
                    print("no: " + str(info.record[index].no) + ", start: " + str(info.record[index].start_time) + ", finish: " + str(info.record[index].finish_time))
...
def concatenate_all_ob(trajs, pa):
    ...
    all_ob = np.zeros(
        (timesteps_total, 1, pa.network_compact_dim),
...
def concatenate_all_ob_across_examples(all_ob, pa):
    ...
    all_ob_contact = np.zeros(
        (total_samp, 1, pa.network_compact_dim),
        dtype=theano.config.floatX)
...
def process_all_info(trajs):
    ...
    for traj in trajs:
        ...
        job_len.append(np.array([traj['info'].record[i].durations[0] for i in xrange(len(traj['info'].record))]))
...
def main():
    ...
    pa.episode_max_length = 300  # 2000
